...
# Join tables, filtering out those with <50 reviews, and select specified columns
...
shinyServer(function(input, output, session) {
        ...
        movies <- reactive({
                ...
                # end for one-side slider
                reviews <- input$reviews
                oscars <- input$oscars
                # ends for two-side slider
...
#                 # Optional: filter by cast member
        ...
        vis <- reactive({
                ...
                movies %>%
                        ggvis(x = xvar, y = yvar) %>%
                        layer_points(size := 150, size.hover := 450,
                        ...
                        scale_nominal("stroke", domain = c("Yes", "No"),
                                      range = c("red", "blue")) %>%
                        set_options(width = 500, height = 600)
        })
